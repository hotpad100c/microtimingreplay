...
public class EntityMoveEvent extends Vec3PosEvent {
    ...
    public MutableComponent fillHoverText() {
        String fromPos = String.format("@[%s,%s,%s]", formatCoord(oldX), formatCoord(oldY), formatCoord(oldZ));
        String toPos = String.format("@[%s,%s,%s]", formatCoord(getX()), formatCoord(getY()), formatCoord(getZ()));

        MutableComponent text = MTRComponent.translatable(
                "mtr.tooltip.entity_move_title",
                "Entity Move @ %s",
                toPos
        ...
        .append(Component.literal("\n").append(MTRComponent.translatable("mtr.tooltip.entity.from", "From: %s", fromPos)).withStyle(ChatFormatting.GRAY))
        .append(Component.literal("\n").append(MTRComponent.translatable("mtr.tooltip.entity.to", "To: %s", toPos)).withStyle(ChatFormatting.GREEN))
        .append(Component.literal(String.format("\nVelocity: (%.2f, %.2f, %.2f)", dx, dy, dz)).withStyle(ChatFormatting.WHITE));
    }
